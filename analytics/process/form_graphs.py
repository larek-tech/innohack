...
def get_one_multipl_records(records: dict, multy_key: str, start_date: str, end_date: str, return_year = True) -> list[analytics_pb2.Record]:
    record_points = []
    start_date = int(start_date)
    end_date = int(end_date)
    for year in range(start_date, end_date + 1, 1):
...
def count_records_percentage(records: list[analytics_pb2.Record]) -> list[analytics_pb2.Record]:
    valid_records = [
        rec for rec in records
        if not math.isnan(rec.y)
    ]
    percent_records = []
    whole_sum = 0
...
def form_graph_info(
        records: dict, 
        multipliers: dict, 
        request: analytics_pb2.Params) -> list[analytics_pb2.Chart]:
    charts = []
    ...
    if start_date == end_date:
        ...
        # single pie
        recs = [
            get_one_param_records(records, 1110, start_date, end_date, return_year=False),
            get_one_param_records(records, 1120, start_date, end_date, return_year=False),
            get_one_param_records(records, 1130, start_date, end_date, return_year=False),
            get_one_param_records(records, 1140, start_date, end_date, return_year=False),
            get_one_param_records(records, 1150, start_date, end_date, return_year=False),
            get_one_param_records(records, 1160, start_date, end_date, return_year=False),
            get_one_param_records(records, 1170, start_date, end_date, return_year=False),
        ]
        recs = [
        ...
        recs = [
            get_one_param_records(records, 1210, start_date, end_date, return_year=False),
            get_one_param_records(records, 1220, start_date, end_date, return_year=False),
            get_one_param_records(records, 1230, start_date, end_date, return_year=False),
            get_one_param_records(records, 1240, start_date, end_date, return_year=False),
            get_one_param_records(records, 1250, start_date, end_date, return_year=False),
            get_one_param_records(records, 1260, start_date, end_date, return_year=False),
        ]
        recs = [
            rec[0] for rec in recs
            if len(rec) > 0
        ...
        recs = [
            get_one_param_records(records, 1310, start_date, end_date, return_year=False),
            get_one_param_records(records, 1320, start_date, end_date, return_year=False),
            get_one_param_records(records, 1340, start_date, end_date, return_year=False),
            get_one_param_records(records, 1350, start_date, end_date, return_year=False),
            get_one_param_records(records, 1360, start_date, end_date, return_year=False),
            get_one_param_records(records, 1370, start_date, end_date, return_year=False),
        ]
        recs = [
            rec[0] for rec in recs
            if len(rec) > 0
        ...
        recs = [
            get_one_param_records(records, 1410, start_date, end_date, return_year=False),
            get_one_param_records(records, 1420, start_date, end_date, return_year=False),
            get_one_param_records(records, 1430, start_date, end_date, return_year=False),
            get_one_param_records(records, 1450, start_date, end_date, return_year=False),
            get_one_param_records(records, 1460, start_date, end_date, return_year=False),
        ]
        recs = [
            rec[0] for rec in recs
            if len(rec) > 0
        ...
        recs = [
            get_one_param_records(records, 1510, start_date, end_date, return_year=False),
            get_one_param_records(records, 1520, start_date, end_date, return_year=False),
            get_one_param_records(records, 1530, start_date, end_date, return_year=False),
            get_one_param_records(records, 1535, start_date, end_date, return_year=False),
            get_one_param_records(records, 1540, start_date, end_date, return_year=False),
            get_one_param_records(records, 1550, start_date, end_date, return_year=False),
        ]
        recs = [
            rec[0] for rec in recs
            if len(rec) > 0
        ...
        # coefs
        # group line
        recs = get_one_multipl_records(multipliers, "Autonomy_Ratio", start_date, end_date)
        if len(recs) > 0:
            charts.append(
                analytics_pb2.Chart(
        ...
        recs = get_one_multipl_records(multipliers, "Capitalization_Ratio", start_date, end_date)
        if len(recs) > 0:
            charts.append(
                analytics_pb2.Chart(
                    title="Динамика показателей финансовой устойчивости",
        ...
        recs = get_one_multipl_records(multipliers, "Investment_Coverage_Ratio", start_date, end_date)
        if len(recs) > 0:
            charts.append(
                analytics_pb2.Chart(
                    title="Динамика показателей финансовой устойчивости",
        ...
        recs = get_one_multipl_records(multipliers, "Inventory_Security_Ratio", start_date, end_date)
        if len(recs) > 0:
            charts.append(
                analytics_pb2.Chart(
                    title="Динамика показателей финансовой устойчивости",
        ...
        recs = get_one_multipl_records(multipliers, "Financial_Dependency_Ratio", start_date, end_date)
        if len(recs) > 0:
            charts.append(
                analytics_pb2.Chart(
                    title="Динамика показателей финансовой устойчивости",
        ...
        recs = get_one_multipl_records(multipliers, "Financial_Leverage_Ratio", start_date, end_date)
        if len(recs) > 0:
            charts.append(
                analytics_pb2.Chart(
                    title="Динамика показателей финансовой устойчивости",
...
def get_analitics_report(request: analytics_pb2.Params) -> analytics_pb2.Report:
    ...
    if request.start_date == request.end_date:
        for k, v in multipliers.items():
            ...
            if value is not None and not math.isnan(value):
                return_multy.append(
                    analytics_pb2.Multiplier(
                        key=MULTYPLIER_NAME[k],
                        value=value
                    )
                )
